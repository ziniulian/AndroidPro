...
ol = {
    ...
    crtDom: function (rid) {
        ...
        t.className = "txt mfs";
        t.innerHTML = "<a href=\"panInfo.html?rid=" + rid + "\">" + rid + "</a>";
    ...
    add: function () {
        if (!this.busy && this.getAjax()) {
            ...
            this.ajx.open("POST", srvurl, true);
            this.ajx.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=utf-8");
            var msg = "srv=crtPlan&rid=" + this.busy + "&user=" + this.user;
            this.ajx.send(msg);
        }
    },
    ...
    del: function (evt) {
        if (!ol.busy && ol.getAjax()) {
            ...
            var msg = "srv=delPlan&rid=" + ol.busy.getAttribute("rid");
            ol.ajx.send(msg);
        }
    },
