...
ol = {
            ...
            if (a.status === 200) {
                ...
                if (o.ok) {
                    ...
                    for (i = 0; i < o.dat.length; i ++) {
                        devs.appendChild(ol.crtDom(o.dat[i].devid, o.dat[i].status));
                    }
                }
            }
            ol.busy = false;
    ...
    cacl: function (evt) {
        if (!ol.busy && ol.getAjax()) {
            ...
            var msg = "srv=clearRecord&rid=" + ol.rid + "&devid=" + ol.busy.getAttribute("devid");
            ol.ajx.send(msg);
        }
    },
    ...
    del: function (evt) {
        if (!ol.busy && ol.getAjax()) {
            ...
            ol.ajx.open("POST", srvurl, true);
            ol.ajx.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=utf-8");
            var msg = "srv=delOneRecord&rid=" + ol.rid + "&devid=" + ol.busy.getAttribute("devid");
            ol.ajx.send(msg);
        }
    },
