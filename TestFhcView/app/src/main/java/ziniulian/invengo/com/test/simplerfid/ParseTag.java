...
public class ParseTag {
	...
	public static Map<String, String> parseXCTF2 (byte[] src) {
...
//		t[j++] = (byte)(t1 + 0x20);
		...
		t1 = ((t1 << 2) | (t2 >> 6));
		t[j++] = (byte)(t1 + 0x20);
		f = new String(new byte[]{t[16]});
		r.put("trainFactory", f);
		r.put("trainFactoryAlias", parseFac(s, f));
		...
		t1 = ((t1 << 4) | (t2 >> 4));
		t[j++] = (byte)(t1 + 0x20);
		s = new String(new byte[]{t[17], t[18], t[19]});
		r.put("trainDate", s);
	...
	public static String parsePro (String pro) {
		return DicPro.getTyp(pro);
	}

	// 翻译制造厂
	public static String parseFac (String pro, String fac) {
		return DicPro.getFac(pro, fac);
	}
}
